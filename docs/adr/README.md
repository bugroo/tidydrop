...
| [ADR-0018](0018-safe-authenticated-dmg-bundle-inspection.md) | Accepted and implemented as a non-shipping foundation | Authenticated DMGs are mounted read-only and their bounded TidyDrop bundle, Universal 2 components and code signature are verified; installation remains gated |
| [ADR-0019](0019-private-recovery-state-snapshot.md) | Accepted and implemented as a non-shipping foundation | Private configuration and SQLite state snapshots force dry-run recovery; app retention and external replacement remain gated |
| [ADR-0020](0020-retained-current-bundle-and-recovery-journal.md) | Accepted and implemented as a non-shipping foundation | Retain and reverify the current bundle and publish a durable recovery journal; replacement and recovery execution remain gated |
| [ADR-0021](0021-external-recovery-helper-and-atomic-swap.md) | Accepted and implemented as a temporary-scope non-shipping foundation | External helper and descriptor-relative atomic install/rollback protocol under `/private/tmp`; installed-app authority remains gated |
| [ADR-0022](0022-recovery-helper-process-kill-harness.md) | Accepted and implemented as a non-shipping fault-injection gate | Real helper subprocess death and fresh-process recovery at all four durable install/rollback boundaries |
| [ADR-0023](0023-schema-bound-dry-run-state-restoration.md) | Accepted and implemented as a temporary-scope non-shipping foundation | Restore authenticated compatible configuration and SQLite state after rollback, always in dry-run and never through personal-file undo |
| [ADR-0024](0024-state-restoration-process-kill-harness.md) | Accepted and implemented as a non-shipping fault-injection gate | Kill and relaunch the helper at all five durable state-restoration boundaries, repeated for 25 abrupt process deaths |
