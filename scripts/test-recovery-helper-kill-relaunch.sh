...
REPETITIONS=${TIDYDROP_RECOVERY_KILL_REPETITIONS:-5}
TEST_NAME=testRecoveryHelperSurvivesProcessKillAtEveryDurableBoundary
...
while [ "$iteration" -le "$REPETITIONS" ]; do
    printf 'Recovery helper kill/relaunch iteration %s/%s\n' "$iteration" "$REPETITIONS"
    env \
...
kill_count=$((REPETITIONS * 4))
printf '%s\n' \
    "Recovery helper subprocess kill/relaunch: PASS ($kill_count forced kills across 4 durable boundaries)"
